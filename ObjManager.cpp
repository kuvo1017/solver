...
void ObjManager::deleteVehicle(Vehicle* vehicle,bool b)
{
  vector<Vehicle*>::iterator itv = find(_vehicles.begin(),
      _vehicles.end(),
      vehicle);
  std::vector<RoadOccupant*>* agents = vehicle->lane()->agents();
  ...
  if (itv!=_vehicles.end())
  {
    ...
    /*
       ...
       if(reinterpret_cast<Vehicle*>(agents->at(i))==vehicle)
       std::cout << "あるよ"<< endl;
     */
  }
  vector<RoadOccupant*>::iterator ita = find(agents->begin(),
      agents->end(),
      reinterpret_cast<RoadOccupant*>(vehicle));
  if (itv!=_vehicles.end())
  {
    //std::cout << "あるよ"<< endl;  
    agents->erase(ita);

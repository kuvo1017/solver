    ...
    for (unsigned int i=0; i<odNodes.size(); i++)
    {
        ...
        if (!(validGTCells.empty()))
        {
            ...
            for (unsigned int j=0; j<validGTCells.size(); j++)
            {
                ...
                {
                    _addNextGenerateTime(validGTCells[j]->begin(),
                                         validGTCells[j]);
                }
            }
        }
...
ODNode* GenerateVehicleController::_decideGoalRandomly(ODNode* start)
{
    ...
    // 目的地のレベルをランダムに選択
    unsigned int r1 = Random::uniform(RAND_MAX) % total;
    ...
    unsigned int r2 = Random::uniform(_goalLevel[goalLevel].size());
    while(true)
    {
        ...
        if (_goalLevel[goalLevel].size()==1)
        {
            ...
            r1 = Random::uniform(RAND_MAX) % total;
            for (unsigned int i=0; i<3; i++)
            {
                if (r1<level[i]*_goalLevel[i].size())
                {
        ...
        r2 = (r2 + Random::uniform(1, _goalLevel[goalLevel].size()))

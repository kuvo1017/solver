...
void RoadMap::writeMapInfo() const
{
    ...
    GVManager::getVariable("RESULT_NODE_INFO_FILE", &fNodeInfo);
    GVManager::getVariable("RESULT_LINK_INFO_FILE", &fLinkInfo);
...
void RoadMap::writeMapInfoWalker() const
{
    ...
    GVManager::getVariable("RESULT_ROAD_ENTITY_INFO_FILE",
                           &fRoadEntityInfo);
    GVManager::getVariable("RESULT_GATEWAY_INFO_FILE",
                           &fGatewayInfo);
    ...
    if (!ofsRoadEntity.fail())
    {
        ...
        const vector<LaneBundle*>* bundles = laneBundles();
        int entityNum = 0;
        CITRMAPENT ite;
        LaneBundle* bundle;

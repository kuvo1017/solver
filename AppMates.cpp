...
struct option AppMates::longOptions[] =
{
    ...
    {"output-generate",      0, 0, 'G'},
    {"no-output-generate",   0, 0, 'g'},
    {"output-instrument",    0, 0, 'M'},
    {"no-output-instrument", 0, 0, 'm'},
    {"verbose",              0, 0, 'Q'},
    {"no-verbose",           0, 0, 'q'},
    {"output-monitor-d",     0, 0, 20},
    {"no-output-monitor-d",  0, 0, 21},
    {"output-monitor-s",     0, 0, 22},
    {"no-output-monitor-s",  0, 0, 23},
...
void AppMates::parseArgument(int argc, char** argv)
{
...
#endif //USE_MINGW
    {
        ...
        case 'm': // 路側器データ出力をoffに(simデフォルトでoff)
            ...
            break;
#ifndef USE_MINGW
        case 20: // 路側器データのうち詳細データの出力をonに
            GVManager::resetFlag("FLAG_OUTPUT_MONITOR_D", true);
            break;
        case 21: // 路側器データのうち詳細データの出力をoffに
            GVManager::resetFlag("FLAG_OUTPUT_MONITOR_D", false);
            break;
        case 22: // 路側器データのうち統計データの出力をonに
            GVManager::resetFlag("FLAG_OUTPUT_MONITOR_S", true);
            break;
        case 23: // 路側器データのうち統計データの出力をoffに
            GVManager::resetFlag("FLAG_OUTPUT_MONITOR_S", false);
            break;
#endif //USE_MINGW
